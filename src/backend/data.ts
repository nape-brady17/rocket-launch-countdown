...
// these links may need to be updated, I am currently getting rate limits

export async function fetchUpcomingLaunches() {
  try {
    const res = await fetch('https://ll.thespacedevs.com/2.2.0/launch/upcoming/?location__ids=27,12&status__ids=1,8&limit=5', {
      mode: 'cors',
    });
...
export async function fetchPreviousLaunches() {
  try {
    const res = await fetch('https://ll.thespacedevs.com/2.2.0/launch/previous/?location__ids=27,12&limit=5', {
      mode: 'cors',
    });
    const data: any = await res.json();
